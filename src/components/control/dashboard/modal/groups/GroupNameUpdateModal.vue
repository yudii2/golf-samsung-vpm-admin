<template>
  <transition name="fade" appear>
    <div class="backdrop" @click.self="closeModal">
      <div class="group-name-update-modal__container">
        <!-- 닫기 버튼 -->
        ...
        <section class="mt">
          <form @submit.prevent="handleUpdateSubmit">
            ...
            <SimpleButton class="mt" buttonText="변경하기" />
          </form>
        </section>
      </div>
    </div>
...
const { getGroups, updateGroupName } = useGroup();

export default {
  name: "GroupNameUpdateModal",
  ...
  computed: {
    currentGroupName() {
      return this.selectedGroup?.data?.groupName;
    },
  ...
  methods: {
    ...
    initData() {
      const { groupName, groupCd } = this.selectedGroup.data;
      this.groupName = groupName;
      this.groupCode = groupCd;
    },

    closeModal() {
      this.setIsShowingGroupNameUpdateModal(false);
    },
    ...
    async handleUpdateSubmit() {
      ...
      if (cleanedGroupName.length === 0) {
        ...
        const { status } = await updateGroupName(
          this.groupCode,
          cleanedGroupName
        );
        if (status && status === "OK") {
...
<style scoped>
.group-name-update-modal__container {
  position: absolute;
  width: 550px;
  top: 50%;
  left: 50%;
