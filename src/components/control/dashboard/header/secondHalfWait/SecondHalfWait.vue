<template>
  <div class="second_wait__container">
    <!-- header -->
    <header>
      <img
        class="mr-1/2"
  ...
  computed: {
    ...
    isValidCourses() {
      ...
      return newCourses.map((newCourse) => {
        const caddies = this.currentCaddies.filter((caddie) =>
          this.isSecondHalfWait(newCourse.courseCode, caddie)
        );
    ...
    isSecondHalfWait() {
      return (courseCode, caddie) => {
        // 10T 대기 여부
        const isSecondWait = caddie.playStep == CADDIE_PLAY_STEP_SECOND_WAIT;
        ...
        return isSecondCourse && isSecondWait && isNotAbnormal;
      };
    },
    gameIsStarted() {
      return (caddie) => {
        ...
        return caddie.startTeeYn === "Y" &&
          caddie.currCourseCd === caddie.secondCourseCd
          ? true
          : false;
      };
    },
...
.second_wait__item:nth-child(odd) {
