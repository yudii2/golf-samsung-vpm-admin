<template>
  <!-- 모달 > 실시간예약 취소 -->
  <section class="te-modal te-realCancel" v-if="onModal">
    <div class="te-body">
      <div class="te-b-body">
...
<script>
export default {
  name: "RealCancelModal",
  created() {
    this.$EventBus.$on("openRealCancel", this.setOnModal);
  },
  beforeDestroy() {
    this.$EventBus.$off("openRealCancel");
  },
  data() {
    return {
      modalInfo: null,
  ...
  methods: {
    setOnModal(info, resolve) {
      document.querySelector("body").classList.add("active-modalRealCancel");
      this.resolve = resolve;
      this.modalInfo = info;
      this.onModal = true;
    },
    setOffModal(response) {
      document.querySelector("body").classList.remove("active-modalRealCancel");
      this.onModal = false;
