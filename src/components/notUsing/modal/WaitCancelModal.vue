<template>
  <!-- 모달 > 대기예약 취소 -->
  <section class="te-modal te-waitCancel" v-if="onModal">
    <div class="te-body">
      <div class="te-b-body">
...
<script>
export default {
  name: "WaitCancelModal",
  created() {
    this.$EventBus.$on("openWaitCancel", this.setOnModal);
  },
  beforeDestroy() {
    this.$EventBus.$off("openWaitCancel");
  },
  data() {
    return {
      modalInfo: null,
  ...
  methods: {
    setOnModal(info, resolve) {
      document.querySelector("body").classList.add("active-modalWaitCancel");
      this.resolve = resolve;
      this.modalInfo = info;
      this.onModal = true;
    },
    setOffModal(response) {
      document.querySelector("body").classList.remove("active-modalWaitCancel");
      this.onModal = false;
